        ...
        <v-col cols="12">
          <v-card>
            <v-card-title class="d-flex justify-space-between align-center">
              ...
              <v-btn color="primary" prepend-icon="mdi-plus" to="/ordem-servico/nova">
                NOVA ORDEM
              </v-btn>
            </v-card-title>
            ...
            <v-card-text>
              <v-list v-if="ordensServico.length > 0">
                <v-list-item v-for="ordem in ordensServico" :key="ordem.id" :value="ordem"
                  ...
                  <template v-slot:append>
                    <v-chip :color="getStatusColor(ordem.status)" size="small" class="mr-2">
                      {{ getStatusText(ordem.status) }}
                    </v-chip>
                    <v-btn icon="mdi-eye" variant="text" size="small" color="primary" class="mr-2"
                      @click.stop="visualizarOrdem(ordem.id)"></v-btn>
                    <v-btn icon="mdi-pencil" variant="text" size="small" color="warning"
                      @click.stop="editarOrdem(ordem.id)"></v-btn>
                  </template>
                </v-list-item>
              </v-list>
      ...
      <v-row class="mt-4">
        <v-col cols="12" md="6">
          <v-card>
            ...
            <v-list lines="two"> 
              <v-list-item v-for="cliente in clientesRecentes" :key="cliente.id" :value="cliente"
                ...
                <template v-slot:append>
                  <v-btn icon="mdi-phone" variant="text" size="small" @click.stop="ligarParaCliente(cliente.telefone)"
                    </v-btn>
                </template>
              </v-list-item>
              
              <v-list-item v-if="clientesRecentes.length === 0">
        ...
        <v-col cols="12" md="6">
          <v-card>
            ...
            <v-list>
              <v-list-item v-for="tipo in tiposServico" :key="tipo.id" :value="tipo">
                <template v-slot:prepend>
              ...
              <v-list-item v-if="tiposServico.length === 0">
                <v-list-item-title class="text-center text-grey">
      ...
      <v-row class="mt-4">
        <v-col cols="12" md="6">
          <v-card>
            ...
            <div v-if="ultimasOrdens.length > 0">
              <v-list>
                <v-list-item v-for="ordem in ultimasOrdens" :key="ordem.id" :value="ordem.id"
                  @click="navegarParaOrdem(ordem.id)" class="mb-2">
                  <template v-slot:prepend>
                  ...
                  <div>
                    ...
                    <div class="text-caption text-medium-emphasis">
                      Data: {{ formatDate(ordem.data_servico) }}
                      <v-chip :color="getStatusColor(ordem.status)" size="x-small" class="ml-2">
                        {{ getStatusText(ordem.status) }}
                      </v-chip>
                    </div>
                  </div>
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="primary" variant="text" to="/ordens-servico">
                Ver Todas
              </v-btn>
            </v-card-actions>
          </v-card>
        ...
        <v-col cols="12" md="6">
          <v-card>
            ...
            <v-list>
              <v-list-item link to="/ordem-servico/nova" class="mb-2">
                <template v-slot:prepend>
              ...
              <v-list-item link to="/clientes" class="mb-2">
                <template v-slot:prepend>
              ...
              <v-list-item link to="/tipos-servico">
                <template v-slot:prepend>
...
const search = ref(''); // <--- ADICIONAR
const clientesRecentes = ref([]); // <--- ADICIONAR (ou buscar de algum lugar)
const tiposServico = ref([]); // <--- ADICIONAR (ou buscar de algum lugar)
